...
ANALYSIS_CONFIG_SCHEMA_V1_0 = Schema(
    {
        ...
        SchemaOptional("segment_config"): [
            {SchemaOptional("config_name"): str, "name_or_index": Or(str, int), "segments": [[Or(str, int)]]}
        ],
...
class SegmentationConfig:
    """Config object that defines segment of the dataset on which bias and explainability metrics must be computed.
    SegmentationConfig can only be defined on numerical or categorical columns."""

    def __init__(self, name_or_index: Union[str, int], segments: [[str, int]], config_name: Optional[str] = None):
        """Initializes SegmentationConfig object

        Args:
            name_or_index (str or int): The name or index of the column in the dataset on which this segment is defined
            segments (List[List[str or int]]): Each List of values represents one segment. If N Lists are provided, we
                generate N+1 segments - the additional segment, denoted as the '__default__' segment, is for the rest
                of the values that are not covered by these lists.

                The values in internal list must be intervals for continuous columns (eg.: "[1, 4]" or "(2, 5]"), or one of
                the categorical values for categorical columns.
                Eg,: For a continuous column, `segments` could be [["[1, 4]", "(5, 6]"], ["(7, 9)"]] - this generates 3
                segments. For a categorical columns with values ("A", "B", "C", "D"), `segments` could be [["A", "B"]].
        ...
        if not segments or not isinstance(segments, List) or not all([isinstance(segment, List) for segment in segments]):
...
class DataConfig:
    ...
    def __init__(
        ...
        if segmentation_config:
            _set([item.to_dict() for item in segmentation_config], "segment_config", self.analysis_config)
